...
1. **拖入自动展开（核心）**：从资源管理器/桌面**按住一个文件拖向屏幕顶部的灵动岛** ——
   一旦拖动进入窗口，灵动岛会**自动展开**成大面板并显示「📎 松开鼠标放入暂存架」蒙层，**松手**即暂存。
   > 提示：拖动要先**碰到收起态的小胶囊**才会触发展开（顶部居中、靠上边缘，Fitts 定律友好但目标较小）；
   > 一旦展开，落点区域就变得很大。若某次没接住，略微在胶囊上方多停一下再松手即可。
...
> **已知边界**：① 系统层「拖入」到分层置顶窗口无法 100% 保证接住，故提供 ＋ / 📋 作为可靠入口；
...
- 拖入通过前端 `useShelfDrag()` 监听 webview `onDragDropEvent`，在拖动进入时置 `dragActive`
  强制展开成落点，松手写入暂存架（详见 `modules/shelf.tsx`）。
