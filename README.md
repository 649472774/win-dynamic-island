...
1. **拖入自动展开（核心）**：从资源管理器/桌面**按住文件拖向屏幕顶部的灵动岛** ——
   ...
   > 原理：应用在 Rust 侧注册了**原生 OLE 拖放目标**（`IDropTarget`）。系统的拖放命中判定**不受**
   > 我们的圆角裁剪（`SetWindowRgn`）或透明像素影响，因此**整个顶部居中的窗口矩形**都是落点区 ——
   > 目标很大、很好接。这也是必须自己实现的原因：wry 内置的拖放事件在**透明（分层）窗口**上不会触发。
...
> **已知边界**：① 「拖入」由原生 `IDropTarget` 接管，落点为整个顶部窗口矩形；个别来源（受 DRM 保护的项、
